...
class ayuda:
    def __init__(self, frame):
        ...
        self.ventana = tk.Toplevel(self.frame)
        ...
        label = tk.Label(self.ventana, text="Información sobre el programa", font=self.titulo)
        label.pack(side="top", fill="x", pady=10)

        label1 = tk.Label(self.ventana, text="Datos del estudiante:", font = self.fuente1)
        label1.pack(side="top", fill="x", pady=10)
        
        label2 = tk.Label(self.ventana, text="-> José David Góngora Olmedo\n-> 202201444\n-> Introducción a la Programación y Computación 2 sección ""D""\n-> Ingenieria en Ciencias y Sistemas\n-> 4to Semestre", font = self.fuente2)
        label2.pack(side="top", fill="x", pady=10)

        label3 = tk.Label(self.ventana, text="Documentación", font = self.fuente1)
        label3.pack(side="top", fill="x", pady=10)

        label4 = tk.Label(self.ventana, text="lolo", font = self.fuente2)
        label4.pack(side="top", fill="x", pady=10)
        
        button = tk.Button(self.ventana, text="Regresar",
            highlightbackground='black', height= 4, width=10, command=self.ventana.destroy)
